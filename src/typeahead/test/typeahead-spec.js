...
describe('typeahead tests', function() {
  var $scope, $compile, $document, $templateCache, $timeout, $window, tempuraPosition, offsetYFake;
  ...
  beforeEach(module('tempura.typeahead'));
  ...
  beforeEach(inject(function(_$rootScope_, _$compile_, _$document_, _$templateCache_, _$timeout_, _$window_, $sniffer, _tempuraPosition_) {
    $scope = _$rootScope_;
    ...
    $timeout = _$timeout_;
    $window = _$window_;
    tempuraPosition = _tempuraPosition_;
  ...
  var triggerKeyDown = function (element, keyCode, options) {
    options = options || {};
    var inputEl = findInput(element);
    var event = {type:"keydown", which:keyCode};
    angular.extend(event, options);
    inputEl.triggerHandler(event);
  };
  ...
  describe('basic functionality', function() {
    it('should open and close typeahead based on matches', function() {
      ...
      expect(activeElement(findDropDown(element).find('li')).attr('id')).toBe(activeOptionId);
    ...
    it('should support changing min-length', function() {
        ...
        var element = prepareInputEl('<div><input ng-model="result" ghs-typeahead="item for item in source | filter:$viewValue" typeahead-min-length="typeAheadMinLength"></div>');
    ...
    it('should clear view value after blur for typeahead-editable="false"', function () {
        ...
        inputEl.triggerHandler("blur"); // input loses focus
        expect($scope.result).toEqual(undefined);
    ...
    it('should go through other validators after blur for typeahead-editable="false"', function () {
        var element = prepareInputEl(
        '<div><form name="form">' +
          '<input name="input" ng-model="result" ghs-typeahead="item for item in source | filter:$viewValue" typeahead-editable="false" required>' +
        '</form></div>');
        var inputEl = findInput(element);
    ...
    it('should clear view value when no value selected for typeahead-editable="false" typeahead-select-on-blur="false"', function () {
        var element = prepareInputEl('<div><input ng-model="result" ghs-typeahead="item for item in source | filter:$viewValue" typeahead-editable="false" typeahead-select-on-blur="false"></div>');
        var inputEl = findInput(element);
    ...
    it('should not clear view value when there is match but no value selected for typeahead-editable="false" typeahead-select-on-blur="true"', function () {
        var element = prepareInputEl('<div><input ng-model="result" ghs-typeahead="item for item in source | filter:$viewValue" typeahead-editable="false" typeahead-select-on-blur="true"></div>');
        var inputEl = findInput(element);
    ...
    it('should support changing the editable property to limit model bindings to matches only', function() {
      ...
      var element = prepareInputEl('<div><input ng-model="result" ghs-typeahead="item for item in source | filter:$viewValue" typeahead-editable="isEditable"></div>');
  ...
  describe('shouldSelect', function() {
    it('should select a match when function returns true', function() {
      ...
      var element = prepareInputEl('<div><input ng-model="result" typeahead-should-select="shouldSelectFn($event)" ghs-typeahead="item for item in source | filter:$viewValue"></div>');
      var inputEl = findInput(element);
    ...
    it('should pass key event into select trigger function', function() {
      ...
      expect($scope.shouldSelectFn.calls.first().args[0].which).toEqual(13);
    });
  });
  ...
  describe('selecting a match', function() {
    ...
    it('should select a match on blur with \'select-on-blur=true\' option', function() {
      var element = prepareInputEl('<div><input ng-model="result" ghs-typeahead="item for item in source | filter:$viewValue" typeahead-select-on-blur="true"></div>');
      var inputEl = findInput(element);
    ...
    it('should select match on click', function() {
      ...
      changeInputValueTo(element, 'b');
      var match = angular.element(findMatches(element)[1]);
      match.triggerHandler("click");
      $scope.$digest();
    ...
    it('should invoke select callback on select', function() {
      ...
      var element = prepareInputEl('<div><input ng-model="result" typeahead-on-select="onSelect($item, $model, $label, $event)" ghs-typeahead="state.code as state.name for state in states | filter:$viewValue"></div>');
    ...
    it('should bind no results indicator as true when no matches returned', inject(function($timeout) {
      ...
      var element = prepareInputEl('<div><input ng-model="result" ghs-typeahead="item for item in loadMatches()" typeahead-no-results="isNoResults"></div>');
      changeInputValueTo(element, 'foo');
    ...
    it('should not focus the input if `typeahead-focus-on-select` is false', function() {
      var element = prepareInputEl('<div><input ng-model="result" ghs-typeahead="item for item in source | filter:$viewValue" typeahead-focus-on-select="false"></div>');
  ...
  describe('select on exact match', function() {
    it('should select on an exact match when set', function() {
      ...
      var element = prepareInputEl('<div><input ng-model="result" typeahead-editable="false" typeahead-on-select="onSelect()" ghs-typeahead="item for item in source | filter:$viewValue" typeahead-select-on-exact="true"></div>');
      var inputEl = findInput(element);
    ...
    it('should not select on an exact match by default', function() {
      $scope.onSelect = jasmine.createSpy('onSelect');
      var element = prepareInputEl('<div><input ng-model="result" typeahead-editable="false" typeahead-on-select="onSelect()" ghs-typeahead="item for item in source | filter:$viewValue"></div>');
      var inputEl = findInput(element);
  ...
  describe('is-open indicator', function () {
      ...
      beforeEach(function () {
          element = prepareInputEl('<div><input ng-model="result" ghs-typeahead="item for item in source | filter:$viewValue" typeahead-is-open="isOpen"></div>');
      });
      ...
      it('should bind is-open indicator as false when a match is clicked', function () {
          ...
          changeInputValueTo(element, 'b');
          expect($scope.isOpen).toBeTruthy();
          var match = findMatches(element).eq(0);
      ...
      it('should bind is-open indicator as false input value smaller than a defined threshold', function () {
          var element = prepareInputEl('<div><input ng-model="result" ghs-typeahead="item for item in source | filter:$viewValue" typeahead-is-open="isToggled" typeahead-min-length="2"></div>');
  ...
  describe('non-regressions tests', function() {
    ...
    describe('', function() {
      ...
      it('issue #1773 - should not trigger an error when used with ng-focus', function() {
        element = prepareInputEl('<div><input ng-model="result" ghs-typeahead="item for item in source | filter:$viewValue" ng-focus="foo()"></div>');
        var inputEl = findInput(element);
    ...
    it('issue #3166 - should set \'parse\' key as valid when selecting a perfect match and not editable', function() {
      var element = prepareInputEl('<div ng-form="test"><input name="typeahead" ng-model="result" ghs-typeahead="state as state.name for state in states | filter:$viewValue" typeahead-editable="false"></div>');
      var inputEl = findInput(element);
  ...
  describe('ng-model-options', function() {
    it('should support getterSetter', function() {
      ...
      var element = prepareInputEl('<div><input name="typeahead" ng-model="result" ng-model-options="{getterSetter: true}" ghs-typeahead="state as state.name for state in states | filter:$viewValue" typeahead-editable="false"></div>');
    ...
    describe('debounce as a number', function() {
      it('should work with selecting via keyboard', function() {
        var element = prepareInputEl('<div><input name="typeahead" ng-model="result" ng-model-options="{debounce: 400}" ghs-typeahead="state as state.name for state in states | filter:$viewvalue"></div>');
        var inputEl = findInput(element);
      ...
      it('should work with select on exact', function() {
        var element = prepareInputEl('<div><input name="typeahead" ng-model="result" ng-model-options="{debounce: 400}" ghs-typeahead="state as state.name for state in states | filter:$viewvalue" typeahead-select-on-exact="true"></div>');
        var inputEl = findInput(element);
      ...
      it('should work with selecting a match via click', function() {
        ...
        changeInputValueTo(element, 'Alaska');
        var match = angular.element(findMatches(element)[0]).find('a').eq(0);
    ...
    describe('debounce as an object', function() {
      it('should work with selecting via keyboard', function() {
        var element = prepareInputEl('<div><input name="typeahead" ng-model="result" ng-model-options="{debounce: {default: 400, blur: 500}}" ghs-typeahead="state as state.name for state in states | filter:$viewvalue"></div>');
        var inputEl = findInput(element);
      ...
      it('should work with select on exact', function() {
        var element = prepareInputEl('<div><input name="typeahead" ng-model="result" ng-model-options="{debounce: {default: 400, blur: 500}}" ghs-typeahead="state as state.name for state in states | filter:$viewvalue" typeahead-select-on-exact="true"></div>');
        var inputEl = findInput(element);
      ...
      it('should work with selecting a match via click', function() {
        ...
        changeInputValueTo(element, 'Alaska');
        var match = angular.element(findMatches(element)[0]);
      ...
      it('should work when blurring and select on blur', function() {
        var element = prepareInputEl('<div><input name="typeahead" ng-model="result" ng-model-options="{debounce: {default: 400, blur: 500}}" ghs-typeahead="state as state.name for state in states | filter:$viewvalue" typeahead-select-on-blur="true"></div>');
        var inputEl = findInput(element);
  ...
  describe('append to', function() {
    it('append typeahead results to element', function() {
      $document.find('body').append('<elem id="myElement"></elem>');
      $scope.myElement = $document.find('elem');
      var element = prepareInputEl('<div><input name="input" ng-model="result" ghs-typeahead="item for item in states | filter:$viewValue" typeahead-append-to="myElement"></div>');
      changeInputValueTo(element, 'al');
      expect($document.find('elem')).toBeOpenWithActive(2, 0);
      $document.find('elem').remove();
    });
  });
  ...
  describe('append to body', function() {
    ...
    it('should have right position after scroll', function() {
      //Faking this bc scrolling is not working in PHANTOMJS
      tempuraPosition.offset = function(){
        return {
          top: offsetYFake
        }
      }
  ...
  describe('focus first', function() {
    ...
    it('should not focus the first element until keys are pressed', function() {
      var element = prepareInputEl('<div><input ng-model="result" ghs-typeahead="item for item in source | filter:$viewValue" typeahead-focus-first="false"></div>');
      changeInputValueTo(element, 'b');
      expect(element).toBeOpenWithActive(2, -1);
  ...
  it('should not capture enter or tab when an item is not focused', function() {
    ...
    var element = prepareInputEl('<div><input ng-model="result" ng-keydown="keyDownEvent = $event" ghs-typeahead="item for item in source | filter:$viewValue" typeahead-on-select="onSelect($item, $model, $label)" typeahead-focus-first="false"></div>');
    changeInputValueTo(element, 'b');
  ...
  describe('minLength set to 0', function() {
    it('should open typeahead if input is changed to empty string if defined threshold is 0', function() {
      var element = prepareInputEl('<div><input ng-model="result" ghs-typeahead="item for item in source | filter:$viewValue" typeahead-min-length="0"></div>');
      changeInputValueTo(element, '');
      expect(element).toBeOpenWithActive(3, 0);
    });
...
describe('typeahead tests', function() {
  it('should allow directives in template to require parent controller', function() {
    module('tempura.typeahead');
    module(function($compileProvider) {
      $compileProvider
        .directive('ghsCustomParent', function() {
          return {
            controller: function() {
              this.text = 'foo';
            }
        ...
        .directive('ghsCustomDirective', function() {
          return {
            require: '^ghsCustomParent',
            link: function(scope, element, attrs, ctrl) {
    ...
    inject(function($compile, $rootScope, $sniffer, $templateCache) {
      ...
      $templateCache.put('uib/template/typeahead/template/typeahead-match.html', '<ghs-custom-directive>{{text}}</ghs-custom-directive>');
      ...
      element = $compile('<div ghs-custom-parent><input ng-model="result" ghs-typeahead="state.code as state.name + state.code for state in states" typeahead-template-url="uib/template/typeahead/template/typeahead-match.html"></div>')($scope);
      $rootScope.$digest();
      ...
      inputEl.triggerHandler($sniffer.hasEvent('input') ? 'input' : 'change');
      $scope.$digest();
      expect(element.find('li').eq(0).find('ghs-custom-directive').eq(0).text()).toEqual('foo');
    });
  });
});